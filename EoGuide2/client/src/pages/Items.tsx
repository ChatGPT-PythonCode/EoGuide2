...
export default function Items() {
  ...
  const { data: itemGuides, isLoading: isGuidesLoading } = useGuides("item");
  ...
  const results = useMemo(() => {
    ...
    return rows
      .filter((r) => {
        ...
        const name = getIndexName(r, "Item").toLowerCase();
    ...
    <div className="min-h-screen bg-background pb-20">
      <header className="bg-secondary/20 border-b border-border/50 py-12">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl font-display font-bold text-white mb-2">Items</h1>
          <p className="text-muted-foreground">Search items from the Exile EOR API and browse item guides.</p>
      ...
      <div className="container mx-auto px-4 py-10 space-y-12">
        {/* API Results */}
        <section>
          <h2 className="text-2xl font-bold mb-4">Item Lookup</h2>

          {!rows ? (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Loader2 className="w-4 h-4 animate-spin" /> Loading item index…
            </div>
          ) : q.trim().length < 2 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {results.map((r) => {
                ...
                const name = getIndexName(r, "Item");
                return (
        ...
        <section>
          ...
          {isGuidesLoading ? (
          ...
          ) : itemGuides && itemGuides.length > 0 ? (
            ...
            <div className="text-center py-12 bg-card rounded-3xl border border-border/50 border-dashed">
              <p className="text-muted-foreground text-lg">No item guides yet. Add some from the admin panel.</p>
            </div>
          )}
        </section>
      </div>
